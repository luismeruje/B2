    ...
    for(nc = 0; nc < 4; nc+= 1){
    	...
    	while((counter_jogadas < counter[nc]) && (counter_tree < 40) && ((tree->nextN[nc][counter_tree]) != NULL))
            for(counter_tree = 0; ((tree->nextN[nc][counter_tree]) != NULL) && (counter_tree < 40); counter_tree += 1)
                if((tree->nextN[nc][counter_tree]->t) > 3000 && (jogadas[nc][counter_jogadas] == (tree->nextN[nc][counter_tree]-> estado))){
                    if((tree->nextN[nc][counter_tree]->r)/((float)(tree->nextN[nc][counter_tree]->t))>=wr){
                        wr = (tree->nextN[nc][counter_tree]->r)/((float)(tree->nextN[nc][counter_tree]->t));
                        play =tree->nextN[nc][counter_tree];
                    }
                    counter_jogadas++;
                }
    }
