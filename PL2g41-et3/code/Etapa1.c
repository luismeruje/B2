...
void imprime_jogadas(DATABASE data, char * path){
	...
	int jog;
	int ind;
	for(x=400,y=160,jog=2;jog>=0;jog-=2,y+=145){ //Pôr a ficar o jogador a somar?
        for(ind=0;ind<52;ind++){
            n = ind/13;
            v = ind%13;
    ...
    for(x=200,y=190,jog=3;jog>0;jog-=2,x+=420){//Pôr a ficar o jogador a somar?
        for(ind=0;ind<52;ind++){
            n = ind/13;
            v = ind%13;
            if(carta_existe(data.jogadas[jog],n,v)){
...
void imprime_maos (DATABASE data, char * path){
	...
	int jog;
	int ind;
	for(y = 430, jog = 0; jog < 3; jog+=2, y -= 400)
