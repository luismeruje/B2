...
MCtree choosePlay(MCtree tree, DATABASE * data){
    ...
    MAO jogadas[4][40];
    memset(jogadas,0,8*4*40);
    MCtree play_t = NULL, play_r = NULL, play = NULL;
    int counter[4] = {0};
    ...
    for(nc = 0; nc < 4; nc+= 1){
    	...
    	while((counter_jogadas < counter[nc]) && (counter_tree < 40) && ((tree->nextN[nc][counter_tree]) != NULL))
            for(counter_tree = 0; ((tree->nextN[nc][counter_tree]) != NULL) && (counter_tree < 40); counter_tree += 1) //se chega a um nodo que é NULL e o counter_jogadas ainda n acabou..
    ...
    if(play_r == play_t)
        play = play_r;
    else if((play_r != NULL) && (play_r->t) > 1000) //testar qual tem win rate maior? ==> não é neccessário, vai ser sempre play_r
        play = play_r;
    else
        play = play_t;
    return play;
